...
class Arbol:
    ...
    def busqueda_amplitud(self):
        self.visitados = set()
        self.pila = []
        self._busqueda_amplitud(self.raiz)

    def busqueda_profundidad(self):
        self.visitados = set()
        self._busqueda_profundidad(self.raiz)

    def busqueda_primero_mejor_recursiva(self):
        # Inicializar memoria para la busqueda
        memoria = Nodo('')
        self.solucion = []
        self.fn_final = 0
        self.coste_final = 0
        self._busqueda_primero_mejor_recursiva(memoria, self.raiz, 'Amazonas')
    ...
    def _busqueda_amplitud(self, node):
    ...
    def _busqueda_profundidad(self, nodo):
        if nodo not in self.visitados:
            print(nodo.dato, end=", ")
            self.visitados.add(nodo)
            for conexion in nodo.conexiones:
                self._busqueda_profundidad(conexion.nodo)

    # Busqueda recursiva primero el mejor
    def _busqueda_primero_mejor_recursiva(self, memoria, nodo, meta):
        ...
        if nodo.dato == meta:
            ...
            if len(nodo.conexiones) != 0:
                    ...
                    if len(self.solucion) == 0:
                        if len(conexiones) < 2:
                            ...
                            if conexion1.fn > memoria.fn:
                                ...
                                self._busqueda_primero_mejor_recursiva(
                                    Nodo('', heuristica=float("inf")), memoria, meta)
                            else:
                                self.fn_final += conexion1.fn
                                self.coste_final += conexion1.coste
                                self._busqueda_primero_mejor_recursiva(memoria, conexion1.nodo, meta)
                        else:
                            conexion1 = conexiones.pop(0)
                ...
                self._busqueda_primero_mejor_recursiva(memoria, memoria, meta)
